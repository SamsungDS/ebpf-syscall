 ...
 * (e.g. 8x VLLM::Worker_TP).  During capture those processes submit I/O
 * concurrently, so the block layer can plug + merge requests for
 * adjacent sectors into fewer, larger queued requests.
 *
 * A single serial dispatcher replays every record one-at-a-time, so the
 * block layer almost never has two adjacent requests in flight at once
 * and cannot merge them — producing many small queued requests instead
 * of few large ones (the Q-size / IO-unplug divergence seen in the
 * blktrace comparison).  Total bytes are preserved; the *shape* is not.
 *
 * Fix: give each captured PID its own dispatcher thread and ring buffer
 * so concurrent producers in the trace become concurrent issuers on
 * replay.  The fd_map / mmap tables are already keyed per-PID, so this
 * is safe.  All workers pace against a single shared replay anchor
 * (CLOCK_MONOTONIC at t0) so cross-process ordering is preserved.
 * ------------------------------------------------------------------ */
   ...
   /* ── get per-pid fd_map ── */
	...
	case 20: /*writev*/
	        if (replayed_fd < 0) {
                ...
                * Use fixed 4096 per vector — matches fio bs=4k default and
                * is sufficient to reproduce the I/O pattern faithfully.
                */
                size_t iov_len = 4096;
