...
#   nvme_setup_cmd      → optional NVMe command setup tracepoint
#   nvme_complete_rq    → optional NVMe command completion tracepoint
#
...
class FtraceIOCollector:
    ...
    # Loose patterns for NVMe tracepoints.  The exact trace string varies
    # across kernel releases, so keep these intentionally tolerant.
    # Example setup lines commonly look like:
    #   nvme_setup_cmd: nvme0: nvme0n1 qid=2, cmdid=17, nsid=1, ... cmd=(nvme_cmd_read ...)
    # Example completion lines commonly look like:
    #   nvme_complete_rq: nvme0: nvme0n1 qid=2, cmdid=17, status=0x0
    _RE_NVME_SETUP = re.compile(
        r'nvme_setup_cmd:\s+nvme(?P<ctrl>\d+):\s+'
        r'(?:(?P<disk>nvme\d+n\d+(?:p\d+)?)\s+)?'
        r'.*?\bqid=(?P<qid>\d+).*?\bcmdid=(?P<cmdid>\d+)'
        r'(?:.*?\bnsid=(?P<nsid>\d+))?'
        r'(?:.*?\bcmd=\((?P<cmd>[^)]*)\))?',
        re.IGNORECASE
    )
    _RE_NVME_COMPLETE = re.compile(
        r'nvme_complete_rq:\s+nvme(?P<ctrl>\d+):\s+'
        r'(?:(?P<disk>nvme\d+n\d+(?:p\d+)?)\s+)?'
        r'.*?\bqid=(?P<qid>\d+).*?\bcmdid=(?P<cmdid>\d+)'
    ...
    def _normalize_nvme_device_name(device: str) -> str:
        """Map NVMe generic char namespace names to block namespace names.

        io_uring_cmd passthrough workloads commonly use /dev/ng0n1, while
        block tracepoints and /sys/block are keyed on /dev/nvme0n1.
        """
        if re.match(r'^ng\d+n\d+$', device):
            return 'nvme' + device[2:]
        return device
    ...
    def __init__(self, device: str, duration: int,
        ...
        self.ctrl_id = int(m_ctrl.group(1)) if m_ctrl else None
        self.duration = duration
        ...
        if not dev_path.exists():
            raise RuntimeError(f'/dev/{self.device} not found')
        st = dev_path.stat()
        self.major = os.major(st.st_rdev)
        self.minor = os.minor(st.st_rdev)
        self.allowed_block_devs = self._allowed_block_devs()
        ...
        if self.trace_nvme_cmds and not self._nvme_tracepoints_available():
            raise RuntimeError(
                'NVMe tracepoints are not available under tracefs. Expected:\n'
                '  events/nvme/nvme_setup_cmd/enable\n'
                '  events/nvme/nvme_complete_rq/enable\n'
                'Check kernel config/version or available_events.'
            )

        if self.requested_device != self.device:
            print(f"  Device  : /dev/{self.requested_device} → /dev/{self.device}  "
                  f"[{self.major}:{self.minor}]")
        else:
            print(f"  Device  : /dev/{self.device}  [{self.major}:{self.minor}]")
    ...
    def _event_exists(self, system: str, event: str) -> bool:
        return self._tf('events', system, event, 'enable').exists()

    def _nvme_tracepoints_available(self) -> bool:
        return (self._event_exists('nvme', 'nvme_setup_cmd') and
                self._event_exists('nvme', 'nvme_complete_rq'))

    def _allowed_block_devs(self) -> set:
        """Return exact major/minor pairs accepted by the Python filter.

        For a namespace device, include its partitions so I/O issued through
        /dev/nvmeXnYpZ is still attributed to the namespace.  For an explicit
        partition, keep the filter exact.
        """
        allowed = {(self.major, self.minor)}
        if re.search(r'p\d+$', self.device):
            return allowed

        sys_block = Path('/sys/block') / self.device
        if sys_block.exists():
            for devfile in sys_block.glob(f'{self.device}p*/dev'):
                try:
                    maj_s, min_s = devfile.read_text().strip().split(':')
                    allowed.add((int(maj_s), int(min_s)))
                except Exception:
                    pass
        return allowed
    ...
    def _classify_rwbs(rwbs: str) -> str:
        ...
        if rwbs.startswith('W'):
            return 'write'
        if rwbs.startswith('D'):
            return 'discard'
        if rwbs.startswith('F'):
            return 'flush'
        return 'other'

    @staticmethod
    def _classify_nvme_cmd(cmd: str) -> str:
        cmd = (cmd or '').lower()
        if 'read' in cmd:
            return 'read'
        if 'write' in cmd:
        ...
        if 'flush' in cmd:
            return 'flush'
        if 'discard' in cmd or 'dsm' in cmd or 'deallocate' in cmd:
            return 'discard'
        if 'identify' in cmd:
            return 'identify'
        return 'other'

    def _nvme_line_is_for_device(self, disk: str, ctrl: str = None) -> bool:
        # If the kernel tracepoint includes a namespace disk name, match it.
        if disk:
            return disk == self.device
        # Some kernels/commands emit controller-only NVMe trace lines.  In that
        # case, match the controller ID parsed from nvmeXnY where possible.
        if ctrl is not None and self.ctrl_id is not None:
            try:
                return int(ctrl) == self.ctrl_id
            except ValueError:
                return False
        return True
    ...
    def _save_restore(self) -> dict:
        ...
        if self.trace_nvme_cmds:
            paths.extend([
                ('nvme_setup_enable', ('events','nvme','nvme_setup_cmd','enable')),
                ('nvme_comp_enable',  ('events','nvme','nvme_complete_rq','enable')),
            ])
    ...
    def collect(self) -> dict:
        ...
        #
        # For a whole namespace device (nvme4n1) we filter by major only in
        # tracefs to reduce ring-buffer volume, then filter by exact
        # namespace/partition major:minor pairs in Python.
        ...
        try:
            ...
            # Try kernel-side block filter (reduces ring buffer volume)
            ...
            # Enable block events used by the existing analyzer
            self._write('events','block','block_rq_issue','enable',    value='1')
            self._write('events','block','block_rq_complete','enable',  value='1')

            # Optional true NVMe command-level tracepoints for io_uring_cmd
            # passthrough and other NVMe submissions.
            if self.trace_nvme_cmds:
                self._write('events','nvme','nvme_setup_cmd','enable', value='1')
                self._write('events','nvme','nvme_complete_rq','enable', value='1')
        ...
        if self.trace_nvme_cmds:
            print(f"  ✓  ftrace NVMe command tracepoints enabled")
        print(f"     kernel filter : {dev_filter}")
        print(f"     python filter : {(sorted(self.allowed_block_devs))}")
        ...
        raw_fh = None
        if self.raw_trace_output:
            self.raw_trace_output.parent.mkdir(parents=True, exist_ok=True)
            raw_fh = open(self.raw_trace_output, 'w', encoding='utf-8', errors='replace')
                ...
                with open(pipe_path, 'r', errors='replace') as pipe:
                    while not stop_flag.is_set():
                        ...
                        if m:
                            ...
                            dev_pair = (int(maj), int(minn))
                            if dev_pair not in self.allowed_block_devs:
                                continue
                            if raw_fh:
                                raw_fh.write(line); wrote_raw = True
                            nr     = int(nrs)
                            bytes_ = int(bstr) if int(bstr) > 0 else nr * SECTOR_BYTES
                            sector = int(sec)
                            op     = self._classify_rwbs(rwbs)
                        ...
                        if m:
                            ...
                            nr     = int(nrs)
                            op     = self._classify_rwbs(rwbs)
                            entry  = inflight.pop((sector, nr), None)
                        ...
                        if self.trace_nvme_cmds:
                            m = self._RE_NVME_SETUP.search(line)
                            if m:
                                g = m.groupdict()
                                if not self._nvme_line_is_for_device(g.get('disk'), g.get('ctrl')):
                                    continue
                                if raw_fh and not wrote_raw:
                                    raw_fh.write(line); wrote_raw = True
                            ...
                            if m:
                                ...
                                nvme_summary['complete'] += 1
                                nvme_summary['status_counts'][status] += 1
                                entry = nvme_inflight.pop(key, None)
                                if entry and ts > 0 and entry[0] > 0:
                                    lat_ns = int((ts - entry[0]) * 1e9)
                                    if lat_ns > 0:
        ...
        try:
            ...
            if self.trace_nvme_cmds:
                self._write('events','nvme','nvme_setup_cmd','enable', value='0')
                self._write('events','nvme','nvme_complete_rq','enable', value='0')
        except Exception:
            pass
        self._restore(state)
        ...
        if self.trace_nvme_cmds:
            ...
            print("  NVMe commands  : "
                  ...
                  f"matched={nvme_summary['matched']:,}")
            if nvme_summary['cmd_counts']:
        ...
        return dict(
            ...
            nvme_trace  = dict(
                ...
                complete      = nvme_summary['complete'],
...
def main():
    ...
    p.add_argument('--trace-nvme-cmds', action='store_true',
    ...
    # ── nvme-ebpf ─────────────────────────────────────────────────────────────
        ...
        if args.demo:
            ...
            print("  [nvme-ebpf] Collecting via ftrace block tracepoints"
                  " + optional NVMe command tracepoints ...")
